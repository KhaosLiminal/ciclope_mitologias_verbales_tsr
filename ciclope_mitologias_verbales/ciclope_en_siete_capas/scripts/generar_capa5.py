...
def generar_metaanalisis_tsr(datos_tsr: Dict, prompt_base: str, modelo: str) -> Optional[Dict]:
    ...
    # Seleccionar API según modelo
    if modelo == "minimax":
        resultado = api_minimax(prompt, "minimax-text-01")
    else:
        resultado = api_perplexity(prompt, modelo)
...
def main():
    ...
    # Verificar API keys
    if args.modelo == "minimax" and not MINIMAX_API_KEY:
        print("[ERROR] Para usar MiniMax, configure MINIMAX_API_KEY")
        return
    
    if args.modelo != "minimax" and not PERPLEXITY_API_KEY:
        print("[ERROR] Para usar Perplexity, configure PERPLEXITY_API_KEY")
        return
